...
# Add backend to path so 'src' is the top-level package for relative imports to work
backend_path = os.path.join(os.getcwd(), "graph_rlm", "backend")
...
async def verify():
    ...
    res = await sheaf.check_axiomatic_consistency(
        proposed_code="print('hello')", task_tags=["diagnostic"]
    )
    ...
    res = db.query(
        "MATCH (n:Thought {id: 'test_thought_trace'}) RETURN n.code_hash as hash"
    )
