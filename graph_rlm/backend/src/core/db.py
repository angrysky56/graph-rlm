...
class GraphClient:
    ...
    def create_thought_node(self, thought_id: str, prompt: str, parent_id: Optional[str] = None, prompt_embedding: Optional[List[float]] = None):
        """
        Creates a 'Thought' node in the graph.
        If parent_id is provided, creates a DECOMPOSES_INTO edge from parent to child.
        """
        params: Dict[str, Any] = {"tid": thought_id, "prompt": prompt}

        # Create the node
        cypher = """
        MERGE (t:Thought {id: $tid})
        SET t.prompt = $prompt, t.status = 'pending', t.created_at = timestamp()
        """
        if prompt_embedding:
