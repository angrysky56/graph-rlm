...
function App() {
  ...
  const handleStop = useCallback(async () => {
    ...
    setChatEntries((prev) => [
      ...prev,
      { role: "system", content: "**Stopped by user**", timestamp: Date.now() },
    ]);
  }, []);
    ...
    try {
      ...
      setGraphData((prev) => ({
        ...
        links: prev.links.filter(
          (l: any) => l.source !== thoughtId && l.target !== thoughtId,
        ),
      }));
    } catch (e) {
  ...
  const handleExecute = useCallback(
    (query: string) => {
      ...
      setChatEntries((prev) => [
        ...prev,
        { type: "input", content: query, timestamp: Date.now() },
      ]);
      ...
      const ctrl = api.streamChat(payload, (event) => {
        ...
        if (event.type === "graph_update") {
          ...
          if (
            ...
            setGraphData((prev) => {
              const newData = { ...prev };
              if (action === "add_node") {
                if (!newData.nodes.find((n: any) => n.id === node.id))
                  newData.nodes = [...newData.nodes, node];
        ...
        if (event.ui_target === "CHAT_RESPONSE") {
          if (event.type === "token") {
            setChatEntries((prev) => {
              ...
              if (last && last.isStreaming) {
                return [
                  ...prev.slice(0, -1),
                  { ...last, content: last.content + safeContent },
                ];
              } else {
                return [
                  ...prev,
          ...
          const codeBlock = event.code
            ? `\`\`\`python\n${event.code}\n\`\`\`\n`
            : "";
          const resultBlock = event.content
            ? `**Execution Result:**\n\`\`\`\n${event.content}\n\`\`\``
            : "";
    ...
    >
      <div className="flex h-full relative flex-col">
        ...
        <div className="flex-1 min-h-0 relative flex flex-col">
          <ChatHistory entries={chatEntries} onDelete={handleDeleteThought} />
        </div>
        {/* Input Area */}
