...
export const api = {
  ...
  listModels: async (provider?: string): Promise<Model[]> => {
    ...
    try {
      const res = await apiClient.get("/system/models", {
        headers,
        params: { provider },
      });
      return res.data;
  ...
  unloadModel: async (modelName?: string) => {
    const res = await apiClient.post("/system/unload-model", {
      model: modelName,
    });
    return res.data;
  },
  ...
  streamChat: (payload: any, onEvent: (event: any) => void) => {
    ...
    fetch(`${API_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...payload,
        max_tool_iterations: parseInt(
          localStorage.getItem("NEXUS_MAX_ITERATIONS") || "50",
        ),
      }),
      signal: ctrl.signal,
    }).then(async (response) => {
