    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {flattenedMessages.map((msg, i) => {
                ...
                if (isCode) {
                    return (
                         <div key={i} className="flex flex-col gap-2 mb-4 bg-slate-950/50 p-2 rounded border border-slate-800">
                ...
                if (msg.thinking) {
                     const tagColor = msg.tag ? TAG_COLORS[msg.tag] || 'text-slate-500' : 'text-slate-500';
                     return (
                         <div key={i} className={`text-xs font-mono mb-2 ${tagColor} flex gap-2 ml-10`}>
                             <span className="select-none opacity-50">{msg.tag ? `[${msg.tag}]` : '#'}</span>
                ...
                return (
                    <div key={i} className={`flex gap-4 ${isUser ? 'flex-row-reverse' : ''}`}>
                        {/* Avatar */}
                        <div className={`mt-1 w-8 h-8 rounded shrink-0 flex items-center justify-center border ${isUser
                        ...
                        <div className={`flex flex-col max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}>
                            <div className={`px-4 py-3 rounded-xl border text-sm shadow-sm ${isUser
