...
Runs a multi-phase autonomous software development agency.
"""
...
async def run_code_agency(
    ...
    def load_prompt(role: str) -> str:
        ...
        if not prompt_path.exists():
            logger.warning(
                "Prompt for %s not found at %s, using fallback.", role, prompt_path
            )
    ...
    if phase == "evaluate":
        logger.info("Starting Agency Evaluation on %s...", repo.name)

        # 1. Recall Institutional Knowledge
        memories_result = await call_mcp_tool(
            "memory", "search_memory", {"query": objective, "n_results": 3}
        )
        ...
        """
        # Construct summary prompt for the task
        query_prompt = (
            f"IMPORTANT: Analyze the repository at {repo} (NOT /app). "
            f"Use file reading tools to explore {repo}. Objective: {objective}"
        )
        ...
        summary = report[:500] if report else "Empty report"
        return f"Evaluation Phase Complete.\nReport saved to: {output_file}\n\nSummary:\n{summary}..."

    # --- Phase 2: Plan (Tech Lead) ---
    elif phase == "plan":
        logger.info("Starting Agency Planning on %s...", repo.name)
        ...
        summary = plan[:500] if plan else "Empty plan"
        return f"Planning Phase Complete.\nPlan saved to: {output_file}\n\nSummary:\n{summary}..."

    # --- Phase 3: Execute (Engineer) ---
    elif phase == "execute":
        logger.info("Starting Agency Execution on %s...", repo.name)
    ...
    # --- Phase 4: Verify (QA) ---
        ...
        logger.info("Starting Agency Verification on %s...", repo.name)
