"""
Alpha Tracking Skill.

Identifies high-potential 'alpha' signals by correlating social media momentum
with market data status across social hubs and exchanges.
"""
...
logger = logging.getLogger("graph_rlm.skills.track_alpha_signals")
...
async def track_alpha_signals(top_n: int = 5) -> dict:
    ...
    for q in queries:
        try:
            ...
            # Search failure for one vector shouldn't kill the whole process
            logger.warning("Search vector '%s' failed: %s", q, e)

    # 2. Heuristic Extraction
    # Pattern: $TICKER or uppercase words 3-6 chars long
    raw_tickers = re.findall(
        r"\$([A-Z0-9]{3,8})|(?<=\s)([A-Z]{3,6})(?=\s|\.|\,)", aggregated_text
    )
    ...
    for ticker, mentions in sorted_signals[:10]:
        ...
        try:
            market_res = await call_tool(
                "coinbase-trade-assistant",
                "get_market_data",
                {"product_ids": f"{ticker}-USD"},
            )
            ...
            # Skip errors for specific tickers but log the event
            logger.debug("Failed to fetch market data for %s: %s", ticker, e)
