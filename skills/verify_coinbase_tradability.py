async def verify_coinbase_tradability(token_symbol: str):
    ...
    for product_id in pairs_to_check:
        try:
            ...
            # If one pair fails, try the next one (e.g., USD vs USDC)
            import logging

            logging.getLogger("graph_rlm.skills").debug(
                "Pair lookup failed for %s: %s", product_id, e
            )
            continue
    ...
    return {
        "tradable": False,
        "product_id": None,
        "error": f"No USD/USDC pair found for {symbol}",
    }
