async def logic_gated_ingestion(knowledge_text, domain="General"):
    ...
    raw_p = await verifier_graph.propose_thought(
        type="PREMISE", content=f"New Knowledge Ingestion [{domain}]: {knowledge_text}"
    )
    ...
    result_text = (
        "Logic check passed." if is_valid else "Logic check failed: Data too short."
    )

    raw_tr = await verifier_graph.propose_thought(
        type="TOOL_RESULT", content=result_text, parentIds=[tc_id]
    )
    ...
    if is_valid:
        raw_c = await verifier_graph.propose_thought(
            type="CLAIM",
            content="Promoting knowledge to Verified Backbone.",
            parentIds=[tr_id],
        )
